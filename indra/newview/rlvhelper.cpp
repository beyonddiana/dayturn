...
void RlvForceWear::forceFolder(const LLViewerInventoryCategory* pFolder, EWearAction eAction, EWearFlags eFlags)
{
	...
	{
		LLViewerInventoryItem* pRlvItem = items.operator[](idxItem);
...
void RlvForceWear::done()
{
	...
	if (m_remGestures.size())
	{
		...
		for (S32 idxItem = 0, cntItem = m_remGestures.size(); idxItem < cntItem; idxItem++)
			LLGestureMgr::instance().deactivateGesture(m_remGestures.operator[](idxItem)->getUUID());
		m_remGestures.clear();
	}
	...
	for (addwearables_map_t::const_iterator itAddWearables = m_addWearables.begin(); itAddWearables != m_addWearables.end(); ++itAddWearables)
	{
		...
		{
			LLViewerInventoryItem* pItem = wearItems.operator[](idxItem);
			...
			itAddAttachments != m_addAttachments.end(); ++itAddAttachments)
	{
		...
		for (S32 idxItem = 0, cntItem = wearItems.size(); idxItem < cntItem; idxItem++)
		{
			const LLUUID& idItem = wearItems.operator[](idxItem)->getLinkedUUID();
