...
U32 LLVOAvatar::renderSkinned()
{
	...
	//--------------------------------------------------------------------
		...
		if (!LLDrawPoolAvatar::sSkipOpaque)
		{
			...
			if (!isSelf() || gAgent.needsRenderHead() || LLPipeline::sShadowRender)
			{
                if (isTextureVisible(TEX_HEAD_BAKED) || visually_muted)
                {
					LLViewerJoint* head_mesh = getViewerJoint(MESH_ID_HEAD);
					if (head_mesh)
					{
			...
			if (isTextureVisible(TEX_UPPER_BAKED) || visually_muted)
			{
				LLViewerJoint* upper_mesh = getViewerJoint(MESH_ID_UPPER_BODY);
				if (upper_mesh)
				{
			...
			if (isTextureVisible(TEX_LOWER_BAKED) || visually_muted)
			{
				LLViewerJoint* lower_mesh = getViewerJoint(MESH_ID_LOWER_BODY);
				if (lower_mesh)
				{
	...
	if( num_params > 1)
	{
		...
		for( S32 i = 0; i < num_params; i++ )
		{
			...
			if (slam_params || is_first_appearance_message || (param->getWeight() != newWeight))
			{
				...
				{
					interp_params = TRUE;
					param->setAnimationTarget(newWeight, TRUE);
				}
			}
		}
		const S32 expected_tweakable_count = getVisualParamCountInGroup(VISUAL_PARAM_GROUP_TWEAKABLE) +
		...
		if (params_changed)
		{
			...
			if( old_sex != new_sex )
			{
				updateSexDependentLayerSets(TRUE);
			}	
		}
