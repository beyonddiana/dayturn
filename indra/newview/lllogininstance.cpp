...
void LLLoginInstance::connect(LLPointer<LLCredential> credentials)
{
    ...
    if (uris.size() < 1)
    {
        LL_WARNS() << "Failed to get login URIs during connect. Unable to connect you!" << LL_ENDL;
        return;
    }	
...
void LLLoginInstance::handleLoginFailure(const LLSD& event)
{
	...
	{
		LL_INFOS("LLLogin") << "LLLoginInstance::handleLoginFailure ToS" << LL_ENDL;
	...
	{	
		LL_INFOS("LLLogin") << "LLLoginInstance::handleLoginFailure attemptComplete" << LL_ENDL;
		attemptComplete();
	}	
}
